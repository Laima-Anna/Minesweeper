...
public class MainClass {
    ...
    public static void openAround(int x, int y, UserBoard board, Board realBoard, List<List<Integer>> checked) {
        ...
        if (numbers[4] != -9) board.getBoard()[x][y + 1] = realBoard.getBoard()[x][y + 1];
        if (numbers[5] != -9) board.getBoard()[x + 1][y - 1] = realBoard.getBoard()[x + 1][y - 1];
        if (numbers[6] != -9) board.getBoard()[x + 1][y] = realBoard.getBoard()[x + 1][y];
        if (numbers[7] != -9) board.getBoard()[x + 1][y + 1] = realBoard.getBoard()[x + 1][y + 1];
        ...
        uus6.add(x);
        uus6.add(y + 1);
        if (numbers[4] == 0 && !checked.contains(uus6)) openAround(x, y + 1, board, realBoard, checked);

        List<Integer> uus7 = new ArrayList<>();
        uus7.add(x + 1);
        uus7.add(y-1);
        if (numbers[5] == 0 && !checked.contains(uus7)) openAround(x + 1, y-1, board, realBoard, checked);

        List<Integer> uus8 = new ArrayList<>();
        uus8.add(x + 1);
        uus8.add(y);
        if (numbers[6] == 0 && !checked.contains(uus8)) openAround(x + 1, y, board, realBoard, checked);

        List<Integer> uus9 = new ArrayList<>();
        uus9.add(x+1);
        uus9.add(y +1);
        if (numbers[7] == 0 && !checked.contains(uus9)) openAround(x+1, y + 1, board, realBoard, checked);
    }
}
